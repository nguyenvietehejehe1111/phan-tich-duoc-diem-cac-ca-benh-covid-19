...
# Kiểm tra danh sách các cột và kiểu dữ liệu
print("Các cột trong DataFrame:", df.columns)
print("Kiểu dữ liệu các cột:\n", df.dtypes)

# Chuyển các cột có thể là Categorical sang object để tránh lỗi
...
df['Phường xã'] = df['Phường xã'].astype(str).str.strip().str.title()  # Điều chỉnh tên cột nếu cần

for col in ['Tỉnh/TP', 'Huyện', 'Phường xã']:
    df[col] = df[col].replace('Nan', 'Không xác định').fillna('Không xác định')

# 5. Mã bệnh nhân COVID19
df = df.dropna(subset=['Mã bệnh nhân COVID19'])

# 6. Ngày chuẩn đoán xác định
df['Ngày chẩn đoán xác định'] = pd.to_datetime(df['Ngày chẩn đoán xác định'], errors='coerce')
...
df['Thời gian khai báo sức khỏe gần nhất'] = pd.to_datetime(df['Thời gian khai báo sức khỏe gần nhất'], errors='coerce')
...
df.to_excel('du_lieu_sach.xlsx', index=False)
